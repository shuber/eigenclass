...
Everything in Ruby is an object. Every object has an <tt>eigenclass</tt>.
...
Evaluating code within the context of an object's <tt>eigenclass</tt> allows us to do some pretty cool things, like define a class level attribute accessor.
  ...
  puts SomeObject.testing.inspect #=> :example

This gem provides some convenience methods for defining class level methods.
...
You can also use the <tt>eigenclass_exec</tt> method if you need to evaluate code in an object's <tt>eigenclass</tt> with some additional arguments. Check out the {BasicObject#instance_exec}[http://ruby-doc.org/core-1.9.3/BasicObject.html#method-i-instance_exec] documentation for examples.
